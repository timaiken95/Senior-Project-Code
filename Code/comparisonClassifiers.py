...
def trainSVMEyes(badfile, okfile, saveLocation):
    ...
    trainingData = np.float32(np.vstack((badDataTraining, okDataTraining)))[:,0:2]
...
def trainLinearEyes(badfile, okfile, saveLocation):
...
def testEyesSpecial(badfile, okfile, saveLocation):
    
    svm = cv2.ml.SVM_load(saveLocation)
    
    badData = np.fromfile(badfile).reshape((-1, 8))
    badDataTesting = badData[int(badData.shape[0] * 0.8):,0:2]
    badClassTesting = np.ones(badDataTesting.shape[0])
    
    okData = np.fromfile(okfile).reshape((-1,8))
    okDataTesting = okData[int(okData.shape[0] * 0.8):,0:2]
